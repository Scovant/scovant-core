...
`/plugin marketplace add Scovant/scovant-core` → `/plugin install scovant@scovant`)
with skills `scan`, `explain`, `ci`, `cloud`. `ruleset_version` is
UNCHANGED — no check changed. Ruleset `2026.10`, report schema `1.1`.

### Added
- Claude Code plugin: two MCP servers (`scovant-core` local via `uvx`,
  `scovant-cloud` remote over the user's own API key), four skills, offline
  references generated by `python -m scovant_core.docs --plugin`.
- MCP `scan_site(format=findings|summary|json|markdown, allow_private_networks=False)`.
  `findings` is the new default: non-PASS findings with evidence, PASS ids
  only; `json` is byte-identical to the previous full report.
- `utm_medium=claude-code` for the plugin's CTA.

### Fixed
- The `scan_site` tool description said "45 checks"; it now derives the
